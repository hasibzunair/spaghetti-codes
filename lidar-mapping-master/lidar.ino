...
{ 
  Serial.begin(9600);
  myXservo.attach(9);  // attaches X servo on pin 10 to the servo object 
  myYservo.attach(10);   // attaches Y servo on pin 9 to the servo object  
...
{ 
  while (millis() > 30000);{
  for(posX = 0; posX <= 180; posX += 3) // goes from 30 degrees to 150 degrees 
  {                                  
    for(posY = 0; posY <= 90; posY += 1){  //Y axis sweep //20 140
    myXservo.write(posX);              // tell servo to go to position in variable 'pos' 
    myYservo.write(posY);
    //delay(1);                       // waits for the servo to reach the position
  GetDist();
   } 
   for(posY = 90; posY >=0; posY -=1){ //Y axis sweep
    myXservo.write(posX);              // tell servo to go to position in variable 'pos' 
    myYservo.write(posY);
    //delay(1);                       // waits for the servo to reach the position 
  GetDist();
  ...
  for(posX = 180; posX >=0; posX -=3)     // goes from 150 degrees to 30 degrees 
  { 
    for(posY = 0; posY <= 90; posY += 1){     //Y axis sweep
    myXservo.write(posX);              // tell servo to go to position in variable 'pos' 
    myYservo.write(posY);
    //delay(1);                       // waits for the servo to reach the position
  GetDist();
